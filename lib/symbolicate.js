...
var Symbolicator = {
   ...
   */
  symbolicate: function symbolicate(dSYMPath, report, cb) {
    var metaInfo = {
      'dSYMPath': dSYMPath,
    ...
    */
    ...
    async.mapSeries(images, symbolicateEntry.bind(null, metaInfo), function (err, results) {
      if (err) {
        return cb(err);
      }
...
function symbolicateEntry(metaInfo, entry, cb) {
  ...
  if (object_name === metaInfo.process_name) {
    ...
    var dev_sup_path = Symbolicator.getOpt('base_path', '~/Library/Developer/Xcode/iOS DeviceSupport');
...
function symbolicationReducer(args, symbols) {
  ...
  return function (entry, sym_file, next) {
    if (IS_MAC) {
      inner_args[1] = sym_file.replace(/( |\(|\))/g, '\\$1');
    } else {
