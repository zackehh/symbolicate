...
function symbolicateEntry(metaInfo, entry, cb) {
  ...
  if (object_name === metaInfo.process_name) {
    ...
    if (object_name.endsWith('dylib')) {
      sym_file = path.join(symbol_file, '/Symbols/usr/lib/system/', object_name);
    } else {
      sym_file = path.join(symbol_file, '/Symbols/System/Library/Frameworks', `${object_name}.framework`, object_name);
    }
  }
  ...
  atos.on('close', function (code) {
    ...
    if (code !== 0) {
      ...
      if (strict) {
        cb(new Error(`Error when using ${cmd}`));
      } else {
        cb(null, entry);
      }
    ...
    if (_.isEmpty(stdout)) {
      if (strict) {
        cb(new Error(`Empty result from ${cmd}`));
      } else {
        cb(null, entry);
      }
      return;
